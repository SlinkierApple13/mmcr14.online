...
export class MahjongScene {
  ...
  showRatingUpdate(message: string): void {
    ...
    const style = new TextStyle({
      fontFamily: 'CmuSerif, GameFangsong, sans-serif',
      fontSize: 28,
      fill: 0x000000,
      align: 'center',
    })
  ...
  private createSubComponents(): void {
    ...
    const latencyText = new Text({
      text: '延迟：\u2014',
      style: { fontFamily: 'CmuSerif, GameFangsong, sans-serif', fontSize: 160, fill: 0x000000, align: 'center' },
    })
  ...
  private addPendingButton(ready: boolean, onToggle: () => void): void {
    ...
    const label = new Text({
      text: ready ? '取消准备' : '准备',
      style: { fontFamily: 'CmuSerif, GameFangsong, sans-serif', fontSize: 200, fill: 0x000000, align: 'center' },
    })
  ...
  showRatingResult(
    ...
    if (twoRows) {
      const t1 = new Text({
        ...
        style: { fontFamily: 'CmuSerif, GameFangsong, sans-serif', fontSize: 160 / textScale, fill: 0x000000, align: 'center' },
      })
    ...
    const t2 = new Text({
      text: line2,
      style: { fontFamily: 'CmuSerif, GameFangsong, sans-serif', fontSize: (twoRows ? 110 : 140) / textScale, fill: twoRows ? 0x333333 : 0x000000, align: 'center' },
    })
    t2.anchor.set(0.5)
